...
def Debit(request):
    ...
    if res <= 100:
        messages.info(
            request, "Cannot Debit! Minimum Balance needs to be maintained!!")
        return printing(request, user)
    user.balance = res
...
def Credit(request):
    ...
    if res >= 10000000:
        messages.info(
            request, "Cannot credit! Maximum limit Balance ,it leads transaction revoking")
        return printing(request, user)
    user.balance = res
    user.save()
    Lg = log()
...
def Register(request):
    ...
    if val2 == val3:
        if User.objects.filter(username=val1).exists():
            ...
            Dest.save()
            messages.info(
                request, 'User successfully Created!!...Please Login to continue!')
    else:
